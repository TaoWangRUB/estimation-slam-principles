...
- **cuVSLAM** — NVIDIA, *cuVSLAM: CUDA accelerated visual odometry and mapping*, arXiv:2506.04359; the source release at [github.com/nvidia-isaac/cuVSLAM](https://github.com/nvidia-isaac/cuVSLAM) — **fully open source** (252 .cpp/.cu + 259 headers, no binary blob) under the NVIDIA Community License, whose restriction is platform (NVIDIA hardware) rather than source availability. v17.0.0, 2026-07-21; public API in `libs/cuvslam/cuvslam2.h`, design notes in `DESIGN_CONCEPTS.md` (NVIDIA Community License, full source; up to 32 cameras; Jetson Orin/Thor); and the [Isaac ROS Visual SLAM](https://nvidia-isaac-ros.github.io/repositories_and_packages/isaac_ros_visual_slam/) documentation.
...
The source-level comparison in §4.6 was traced against ORB-SLAM3 `src/ImuTypes.cc`/`Tracking.cc`/`LocalMapping.cc`/`LoopClosing.cc`, cuVSLAM v17.0.0 `libs/{imu,pipelines,sof,sba,slam}`, and VINS-Fusion `vins/src/{factor,estimator,initial}` plus `loop_fusion/src/pose_graph.cpp`. ORB-SLAM3's optimizer surface was read from `include/Optimizer.h`.
